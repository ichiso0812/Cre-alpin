<!DOCTYPE html>
<html lang="fr">
<head>
    <meta desc="">
    <title>Cre'alpin</title>
    <?php include $_SERVER["DOCUMENT_ROOT"] . '/Cre-alpin/includes/head.php';?>
    <link rel="stylesheet" href="./assets/css/home.css"/>
</head>
    <body>
        <div class="wrapper">
            <nav id="menu" class="">
                <div id="menu_header">
                    <a href="#">Se connecter</a>
                </div>
                <div id="menu_header_decoration">
                    <div class="clear_blue"></div>
                    <div class="blue"></div>
                    <div class="yellow"></div>
                    <div class="brown"></div>
                </div>
                <ul id="menu_list" class="components">
                    <li>
                        <a href="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24"><path style="fill:#00AFD4;" d="M11 8h-1v-2h1v1h2v1h-1v1h-1v-1zm2 12v-1h-1v1h1zm-1-15v-1h-2v1h1v1h1v-1zm8-1v6h-1v-1h-4v-5h5zm-1 4v-3h-3v3h3zm-14 2h-1v1h2v-1h-1zm0 3h1v1h1v-3h-1v1h-2v2h1v-1zm5 1v2h1v-2h-1zm4-10h-1v3h1v-3zm0 5v-1h-1v1h1zm3-2h1v-1h-1v1zm-10-1h-1v1h1v-1zm2-2v5h-5v-5h5zm-1 1h-3v3h3v-3zm9 5v1h-1v-1h-2v1h-1v-1h-3v-1h-1v1h-1v1h1v2h1v-1h1v2h1v-2h3v1h-2v1h2v1h1v-3h1v1h1v2h1v-1h1v-1h-1v-1h-1v-1h1v-1h-2zm-11 8h1v-1h-1v1zm-2-3h5v5h-5v-5zm1 4h3v-3h-3v3zm12-3v-1h-1v1h1zm0 1h-1v1h-1v-1h-1v-1h1v-1h-2v-1h-1v2h-1v1h-1v3h1v-1h1v-1h2v2h1v-1h1v1h2v-1h1v-1h-2v-1zm-9-3h1v-1h-1v1zm10 2v1h1v1h1v-3h-1v1h-1zm2 4v-1h-1v1h1zm0-8v-1h-1v1h1zm-2-10h4v4h2v-6h-6v2zm-16 4v-4h4v-2h-6v6h2zm4 16h-4v-4h-2v6h6v-2zm16-4v4h-4v2h6v-6h-2z"/></svg>
                            Scanner un QR code
                        </a>
                    </li>
                    <li>
                        <a href="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24"><path style="fill:#00AFD4;" d="M15.787 7.531c-5.107 2.785-12.72 9.177-15.787 15.469h2.939c.819-2.021 2.522-4.536 3.851-5.902 8.386 3.747 17.21-2.775 17.21-11.343 0-1.535-.302-3.136-.92-4.755-2.347 3.119-5.647 1.052-10.851 1.625-7.657.844-11.162 6.797-8.764 11.54 3.506-3.415 9.523-6.38 12.322-6.634z"/></svg>
                            Faune et flore
                        </a>
                    </li>
                    <li>
                        <a href="">
                            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24"><path style="fill:#00AFD4;" d="M21.103 14.598c-.509-1.504-2.306-2.497-3.806-1.91l-6.235-10.688-11.062 20h20.25c2.067 0 3.75-1.682 3.75-3.75 0-1.774-1.239-3.265-2.897-3.652zm-.853 5.402h-3.26c-1.515-.008-2.505-1.653-1.708-3.009l-3.595-6.334-1.078 1.906-1-1.906-3.026 3.635 4.521-8.344 5.521 9.552c.875-1.781 3.328-.688 2.688 1.104 1.271-.5 2.687.224 2.687 1.646 0 .965-.785 1.75-1.75 1.75zm-2.236-8.579l-2.656-4.625.867-.498 2.656 4.625-.867.498zm3.298 1.579l-2.656-4.625.867-.498 2.656 4.625-.867.498z"/></svg>
                        
                            Projet Mont Blanc
                        </a>
                    </li>
                    <li>
                        <a href="pages/participation.php">
                            <svg width="28" height="28" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd"><path style="fill:#00AFD4;" d="M3.998 14.376v-3.213c0-.771-.479-1.163-.99-1.163-.504 0-1.008.384-1.008 1.163v4.086c0 .796.072 1.179.574 2.092.793 1.441 2.242 3.461 2.242 6.659h5.003s.559-1.759.763-2.942c.411-2.377-.126-3.471-1.11-4.485-1.022-1.053-1.528-1.551-1.996-2.035-1.081-1.121-2.552.194-1.694 1.222.468.561 1.624 1.803 1.901 2.171.268.357-.231.857-.624.447-.573-.599-1.905-2.083-2.365-2.618-.442-.515-.696-.828-.696-1.384zm15.186 9.624h-5.003s-.558-1.759-.762-2.942c-.412-2.377.126-3.471 1.109-4.485 1.022-1.053 1.528-1.551 1.996-2.035 1.081-1.121 2.552.194 1.694 1.222-.467.561-1.624 1.803-1.901 2.171-.268.357.231.857.624.447.573-.599 1.905-2.083 2.365-2.618.443-.515.694-.828.694-1.384v-3.213c0-.771.481-1.163.992-1.163.506 0 1.007.384 1.007 1.163v4.086c0 .796-.073 1.179-.575 2.092-.793 1.441-2.24 3.807-2.24 6.659zm-5.684-8.667l-.792.522c-.103.092-.253.145-.41.145h-.596c-.157 0-.307-.053-.411-.145l-.791-.522h3zm.028-1.333c.184 0 .333.149.333.333 0 .185-.149.334-.333.334h-3.039c-.184 0-.333-.149-.333-.334 0-.184.149-.333.333-.333h3.039zm.329-.667c0-2.669 2.143-3.772 2.143-6.152 0-2.48-2.001-3.848-4.002-3.848-1.999 0-3.998 1.366-3.998 3.848 0 2.38 2.143 3.52 2.143 6.152h3.714zm3.302-4.819l1.8.816-.552 1.215-1.73-.785c.2-.4.366-.81.482-1.246zm-10.318 0c.116.443.277.841.48 1.247l-1.728.784-.552-1.215 1.8-.816zm12.492-2.514v1.333h-2.002c.014-.446-.026-.896-.123-1.333h2.125zm-12.542 0c-.095.436-.135.884-.122 1.33v.003h-2.002v-1.333h2.124zm9.238-2.285l1.656-1.176.772 1.088-1.67 1.184c-.194-.388-.446-.754-.758-1.096zm-8.058-.001c-.307.335-.558.7-.759 1.097l-1.669-1.184.772-1.088 1.656 1.175zm5.912-1.395l.885-1.793 1.195.59-.884 1.793c-.372-.244-.774-.442-1.196-.59zm-3.767-.001c-.421.147-.824.344-1.196.589l-.884-1.791 1.196-.59.884 1.792zm2.551-.277c-.446-.052-.873-.055-1.334-.002v-2.039h1.334v2.041z"/></svg>

                            Participer
                        </a>
                    </li>
                    <li>
                        <a href="">
                            <svg width="28" height="28" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd"><path style="fill:#00AFD4;" d="M12 0c-5.083 0-8.465 4.949-3.733 13.678 1.596 2.945-1.725 3.641-5.09 4.418-3.073.709-3.187 2.235-3.177 4.904l.004 1h23.99l.004-.969c.012-2.688-.093-4.223-3.177-4.935-3.438-.794-6.639-1.49-5.09-4.418 4.719-8.912 1.251-13.678-3.731-13.678m0 1c1.89 0 3.39.764 4.225 2.15 1.354 2.251.866 5.824-1.377 10.06-.577 1.092-.673 2.078-.283 2.932.937 2.049 4.758 2.632 6.032 2.928 2.303.534 2.412 1.313 2.401 3.93h-21.998c-.01-2.615.09-3.396 2.401-3.93 1.157-.266 5.138-.919 6.049-2.94.387-.858.284-1.843-.304-2.929-2.231-4.115-2.744-7.764-1.405-10.012.84-1.412 2.353-2.189 4.259-2.189"/></svg>

                            Profil
                        </a>
                    </li>
                    <li>
                        <a href="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24"><path style="fill:#00AFD4;" d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm1 18h-2v-8h2v8zm-1-12.25c.69 0 1.25.56 1.25 1.25s-.56 1.25-1.25 1.25-1.25-.56-1.25-1.25.56-1.25 1.25-1.25z"/></svg>
                            A propos
                        </a>
                    </li>
                    <li>
                        <a href="">
                        <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 19.9 23.04">
                            <g id="Calque_2" data-name="Calque 2">
                                <g id="Calque_1-2" data-name="Calque 1">
                                <path id="path" d="M19.9,4.26V14.48a4.55,4.55,0,0,1-1.72,3.39l-6.33,4.58a3.38,3.38,0,0,1-3.8,0L1.72,17.87A4.55,4.55,0,0,1,0,14.48V4.26A3.32,3.32,0,0,1,2.21,1.2,15.93,15.93,0,0,1,4.36.6,26.39,26.39,0,0,1,15.55.6a15.93,15.93,0,0,1,2.15.6A3.34,3.34,0,0,1,19.9,4.26Zm-17.81,0V14.48A2.53,2.53,0,0,0,3,16.17l6.33,4.59a1.29,1.29,0,0,0,1.34,0L17,16.17a2.49,2.49,0,0,0,.86-1.69V4.26A1.29,1.29,0,0,0,17,3.18l-.07,0A3.53,3.53,0,0,0,16.51,3a14.11,14.11,0,0,0-1.42-.37,24.4,24.4,0,0,0-10.28,0A14.11,14.11,0,0,0,3.39,3l-.46.15-.08,0a1.24,1.24,0,0,0-.76,1.07Zm4.41,6,1.88,1.88,5-5a1,1,0,0,1,1.48,0,1.06,1.06,0,0,1,0,1.49L9.12,14.35a1,1,0,0,1-1.48,0h0L5,11.73a1,1,0,0,1,0-1.48,1.05,1.05,0,0,1,1.48,0Z" style="fill: #00afd4"/>
                                </g>
                            </g>
                        </svg>
                            CGU
                        </a>
                    </li>
                    <li>
                        <a href="">
                        <svg viewBox="1 0 511 512" width="28" height="28" xmlns="http://www.w3.org/2000/svg"><path style="fill:#00AFD4;" d="m135.738281 361.070312c7.140625 8.425782 6.097657 21.046876-2.328125 28.1875-8.4375 7.148438-21.058594 6.085938-28.1875-2.328124l-100.007812-118c-6.378906-7.523438-6.191406-18.558594 0-25.859376l100.007812-118c7.140625-8.429687 19.761719-9.46875 28.1875-2.328124 8.425782 7.140624 9.46875 19.761718 2.328125 28.1875l-72.097656 85.070312h355.042969c11.046875 0 20.003906 8.953125 20.003906 20s-8.957031 20-20.003906 20h-355.042969zm356.761719-361.070312h-173.820312c-11.046876 0-20 8.953125-20 20s8.953124 20 20 20h153.816406v432h-153.816406c-11.046876 0-20 8.953125-20 20s8.953124 20 20 20h173.820312c11.042969 0 20-8.953125 20-20v-472c0-11.046875-8.957031-20-20-20zm0 0"/></svg>
                            Se déconnecter
                        </a>
                    </li>
                        
                        
                    
                </ul>
            </nav>
            <div id="content">
                <!--map-->
                <div id="map"></div>
                <!--qr code-->
                <div id="qr_code">
                    <a href="pages/scan">
                        <svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 0 24 24"><path style="fill:white;" d="M11 8h-1v-2h1v1h2v1h-1v1h-1v-1zm2 12v-1h-1v1h1zm-1-15v-1h-2v1h1v1h1v-1zm8-1v6h-1v-1h-4v-5h5zm-1 4v-3h-3v3h3zm-14 2h-1v1h2v-1h-1zm0 3h1v1h1v-3h-1v1h-2v2h1v-1zm5 1v2h1v-2h-1zm4-10h-1v3h1v-3zm0 5v-1h-1v1h1zm3-2h1v-1h-1v1zm-10-1h-1v1h1v-1zm2-2v5h-5v-5h5zm-1 1h-3v3h3v-3zm9 5v1h-1v-1h-2v1h-1v-1h-3v-1h-1v1h-1v1h1v2h1v-1h1v2h1v-2h3v1h-2v1h2v1h1v-3h1v1h1v2h1v-1h1v-1h-1v-1h-1v-1h1v-1h-2zm-11 8h1v-1h-1v1zm-2-3h5v5h-5v-5zm1 4h3v-3h-3v3zm12-3v-1h-1v1h1zm0 1h-1v1h-1v-1h-1v-1h1v-1h-2v-1h-1v2h-1v1h-1v3h1v-1h1v-1h2v2h1v-1h1v1h2v-1h1v-1h-2v-1zm-9-3h1v-1h-1v1zm10 2v1h1v1h1v-3h-1v1h-1zm2 4v-1h-1v1h1zm0-8v-1h-1v1h1zm-2-10h4v4h2v-6h-6v2zm-16 4v-4h4v-2h-6v6h2zm4 16h-4v-4h-2v6h6v-2zm16-4v4h-4v2h6v-6h-2z"/></svg>
                        
                    </a>
                </div>
                <!--MENU-->
                <div id="burger_menu">            
                    <svg id="menu_burger" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 4.13 3.74">
                        <title>menu_burger</title>
                        <rect width="4.13" height="3.74" rx="0.97" style="fill: #00afd4"/>
                        <g>
                            <rect x="0.71" y="0.98" width="1.52" height="0.15" rx="0.07" style="fill: #fff"/>
                            <rect x="0.71" y="1.8" width="2.71" height="0.15" rx="0.07" style="fill: #fff"/>
                            <rect x="1.82" y="2.61" width="1.52" height="0.15" rx="0.07" style="fill: #fff"/>
                        </g>
                    </svg>
                </div>
                    
                    
                
                <!--SLIDER-->
                <div class="slider_container">
                    <div class="slider">
                        <div class="slide">
                            
                            <img src="assets/imgs/lieux/planpraz.jpg" alt="Conseil de randonnées">
                            <div class="slider_text">
                                <h2 class="place_name">Planpraz</h2>
                                <p >10km</p>
                            </div>  
                        </div>
                        <div class="slide">
                            <img src="assets/imgs/lieux/camping-montagne-ete.jpg" alt="Randonneur">
                            <div class="slider_text">
                                <h2 class="place_name">Franchard</h2>
                                <p >6km</p>
                            </div>  
                        </div>
                        <div class="slide">
                            <img src="assets/imgs/lieux/randonneurs_aqua.jpg" alt="Personnes en randonnées à Aqua">
                            <div class="slider_text">
                                <h2 class="place_name">Aqua</h2>
                                <p >7km</p>
                            </div>  
                        </div>
                        <div class="slide">
                            <img src="assets/imgs/lieux/rochinos.jpg" alt="Escalade">
                            <div class="slider_text">
                                <h2 class="place_name">Rochinos</h2>
                                <p>8km</p>
                            </div>  
                        </div>
                    </div>
                </div>
            </div>
        </div>
       
        


        
        <?php include $_SERVER["DOCUMENT_ROOT"] .'/Cre-alpin/includes/scripts.php';?>
        <!--MAP-->
        <!--leaflet-->
        <script src="https://unpkg.com/leaflet@1.6.0/dist/leaflet.js" integrity="sha512-gZwIG9x3wUXg2hdXF6+rVkLF/0Vi9U8D2Ntg4Ga5I5BZpVkVxlJWbSQtXPSiUTtC0TjtGOmxa1AJPuV0CPthew==" crossorigin=""></script>
        <!-- la liste des points (à retrouver via Google Map par exemple, en inspectant un point sur la carte) -->
        <script src=<?='/Cre-alpin/assets/js/map/point_list.js' ?>></script>
        <!-- l'instance de la map leafleat  _SERVER["DOCUMENT_ROOT"] . '/Cre-alpin/assets/js/map/point_list.js'-->
        <script src=<?='/Cre-alpin/assets/js/map/map.js' ?>></script>
        <!-- Affichage info  -->
        <script src=<?='/Cre-alpin/assets/js/map/main.js' ?>></script>

        <script src="assets/js/home.js"></script>
    </body>
</html>